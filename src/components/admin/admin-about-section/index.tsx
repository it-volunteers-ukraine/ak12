...
export const AboutSectionAdmin = ({ data }: IAboutSection) => {
  ...
  const handleSubmit = async (values: FormValues) => {
    try {
      const res = await updateAboutMultiLangAction(values);
  ...
  return (
    <>
      <FormWrapper<FormValues>
        ...
        key={data.uk?.mainTitle || data.en?.mainTitle}
      >
        <FormBuilder data={data} config={createAboutFormBuilderConfig(data)} />
      </FormWrapper>
